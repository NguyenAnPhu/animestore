...
<?php
	...
	if(isset($_REQUEST['origin'])||isset($_REQUEST['material'])||isset($_REQUEST['size'])){
			...
			<div class="css-treeview">
				...
				<ul class="tree-list-pad">
					...
					<li><input type="checkbox" id="item-1" checked="checked" /><label for="item-1">Chất Liệu</label>
						<ul>
							<?php
								$ketqua3 = mysqli_query($link,$sql_material);
					...
					<li><input type="checkbox" checked="checked" id="item-2" /><label for="item-2">Xuất Xứ</label>
						<ul>
							<?php
								$ketqua2 = mysqli_query($link,$sql_origin);
								$n2 = mysqli_num_rows($ketqua2);
								if($n2 > 0 ) {
